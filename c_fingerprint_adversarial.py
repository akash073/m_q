...
# =============================================================================
# Retrieves env var 'NUM_TEST_SAMPLES', defaults to 10000 if not set
NUM_TEST_SAMPLES = int(os.getenv("NUM_TEST_SAMPLES", 10))

# Number of FGSM adversarial parts/epsilons previously generated per dataset
# (part 1 -> eps=0.1, ..., part 10 -> eps=1.0), matching the generation script.
NUM_ADVERSARIAL_PARTS = 10
...
# ============================================================
# Wraps a pre-saved .pt file of stacked {"images": ..., "labels": ...}
# tensors (as produced by the FGSM generation script) so it can be indexed
# the same way as a torchvision dataset: base_dataset[i] -> (image, label).
...
def load_adversarial_dataset(splits_dir: Path, prefix: str, part_index: int, epsilon: float) -> TensorPairDataset:
    """Loads one FGSM-perturbed split saved by the generation script."""
    path = splits_dir / f"{prefix}_test_part_{part_index}_fgsm_eps_{epsilon:.1f}.pt"
    if not path.exists():
        raise FileNotFoundError(
            f"Adversarial split not found: {path}\n"
            f"Run the FGSM generation script for this dataset first."
        )
    data = torch.load(path)
    return TensorPairDataset(data["images"], data["labels"])
...
# normalization stats, its own saved model checkpoint prefix (produced by
# the training script as "<name>_cnn.pth" / "<name>_dnn.pth"), and its own
# adversarial-splits directory (produced by the FGSM generation script as
# "<prefix>_test_splits/<prefix>_test_part_N_fgsm_eps_X.pt").
...
def _get_gpu_core_thread():
    ...
    try:
        ...
        cores_per_sm_map = {
            2: 32, 3: 192, 5: 128, 6: 64, 7: 64, 8: 128, 9: 128,
        }
...
def _run_torch_model(model, image_tensor, mean=(0.1307,), std=(0.3081,)):
    ...
    have their own). Works identically whether image_tensor came from a
    clean torchvision sample or a saved FGSM-perturbed tensor -- both are
    already in raw [0, 1] pixel space at this point."""
...
def build_row(
    ...
    return {
        ...
        # FIXED: this is the classical CNN/DNN pipeline, so
        # quantum_computing is always False here regardless of attack
        # status -- it previously (incorrectly) reused this field to mean
        # "is this an adversarial run", which conflated two unrelated
        # concepts. Attack status now lives ONLY in model_under_attack.
        "quantum_computing":            False,
        # FIXED: this was previously always hardcoded to False in
        # collect_for_model regardless of whether the input was clean or
        # adversarial. It's now correctly driven by the `under_attack`
        # argument, so adversarial (FGSM) runs are properly flagged 1.
        ...
        #     the MNISQ quantum-kernel telemetry CSV ---
        "source_qubits":                None,
        "n_qubits":                     None,
        "fidelity":                     None,
...
def backfill_model_metrics(file_path, model_name):
    ...
    df.loc[mask, "model_accuracy"] = float(accuracy)
    df.loc[mask, "model_precision_weighted"] = float(precision_weighted)
    df.loc[mask, "model_recall_weighted"] = float(recall_weighted)
    df.loc[mask, "model_f1_weighted"] = float(f1_weighted)
...
def collect_for_model(
    ...
    print(
        f"\nCollecting {num_samples} edge samples for {dataset_name} / "
        f"{model_name} ({'ADVERSARIAL eps=' + str(attack_epsilon) if under_attack else 'clean'}) "
        f"on {get_hostname()}"
    )
    ...
    if existing_count >= limit:
        ...
        backfill_model_metrics(output_path, model_name)
        return
    ...
    progress_bar = tqdm(
        ...
        desc=f"{model_name}{' [ATTACK]' if under_attack else ''}",
        unit="sample",
        dynamic_ncols=True,
    )
...
def collect_clean_fingerprints():
    """Runs inference on the clean (unperturbed) test set for every
    dataset/model combination -- the baseline, model_under_attack=0."""
    for dataset_name, config in DATASET_CONFIGS.items():
        print(f"\n{'=' * 60}")
        print(f"Dataset: {dataset_name} (CLEAN)")
        print(f"{'=' * 60}")
...
def collect_adversarial_fingerprints():
    """
    Runs inference on every previously generated FGSM-perturbed split for
    every dataset/model combination. model_under_attack=1 for every row
    collected here, and attack_epsilon records which perturbation budget
    was used for that split.
    """
    for dataset_name, config in DATASET_CONFIGS.items():
        prefix = config["checkpoint_prefix"]
        splits_dir = config["splits_dir"]

        for part_index in range(1, NUM_ADVERSARIAL_PARTS + 1):
            epsilon = part_index / 10

            print(f"\n{'=' * 60}")
            print(f"Dataset: {dataset_name} (ADVERSARIAL, eps={epsilon:.1f})")
            print(f"{'=' * 60}")

            try:
                adv_dataset = load_adversarial_dataset(splits_dir, prefix, part_index, epsilon)
            ...
            for model_name in ("CNN", "DNN"):
                collect_for_model(
                    ...
                    mean=config["mean"],
                    std=config["std"],
                    num_samples=len(adv_dataset),
                    flush_every=25,
                    file_name=f"fgsm_eps_{epsilon:.1f}",
                    under_attack=True,
...
def main():
    # 1) Clean baseline fingerprints (model_under_attack = 0)
    #collect_clean_fingerprints()

    # 2) Adversarial (FGSM) fingerprints (model_under_attack = 1)
    #    Requires the FGSM generation script to have been run first for
    #    each dataset, producing "<prefix>_test_splits/*_fgsm_eps_*.pt".
    collect_adversarial_fingerprints()
